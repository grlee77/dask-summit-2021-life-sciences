...
#         self.args_dask_cpu = (image_dask, coords)  # coords_dask?
...
for shape in [(3840, 2160), (7680, 4320), (192, 192, 192), (512, 256, 256)]:
    ...
    for fname, fixed_kwargs, var_kwargs in [
        ...
        (
            "affine_transform3",  # see special case below
            dict(output=None, output_shape=None, prefilter=prefilter),
            dict(mode=modes, order=orders),
        ),
        ...
        if fname == "affine_transform1":
            ...
            fixed_kwargs["offset"] = offset
            index_str = "_case2"
        elif fname == "affine_transform3":
            fname = fname[:-1]
            # define (random) transformation
            matrix = np.eye(ndim) + (np.random.random((ndim, ndim)) - 0.5) / 5.
            offset = (np.random.random(ndim) - 0.5) / 5. * np.array(shape)
            # define resampling options
            fixed_kwargs["output_shape"] = tuple([s // 2 for s in shape])
            fixed_kwargs["matrix"] = matrix
            fixed_kwargs["offset"] = offset
            index_str = "_case3"
        for chunks in all_chunks:

            fixed_kwargs["output_chunks"] = chunks
            ...
            B = InterpolationBench(
                ...
                module_dask=dask_image.ndinterp,
                index_str=index_str,
            )
            results = B.run_benchmark(duration=1)
            all_results = all_results.append(results["full"])
