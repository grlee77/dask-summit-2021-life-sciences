...
from _time import repeat, repeat_dask
...
class ImageBench(object):
    def __init__(
        ...
        self.fixed_kwargs_dask_cpu = self._update_kwargs_arrays(fixed_kwargs, "dask-cpu")
        self.fixed_kwargs_gpu = self._update_kwargs_arrays(fixed_kwargs, "gpu")
        # Remove output_chunks variable for non dask-image functions if present
        self.fixed_kwargs_cpu.pop("output_chunks", None)
        self.fixed_kwargs_gpu.pop("output_chunks", None)
        ...
        if not function_is_generator:
            ...
            self.func_dask_cpu = getattr(module_dask, function_name)
        else:
            ...
            def gen_dask_cpu(*args, **kwargs):
                generator = getattr(module_dask, function_name)(*args, **kwargs)
                return list(generator)
            ...
            self.func_dask_cpu = gen_dask_cpu
            self.func_gpu = gen_gpu

        self.module_name_cpu = module_cpu.__name__
        self.module_name_dask = module_dask.__name__
    ...
    def _update_array(self, array, target="cpu"):
        ...
        elif target == "dask-cpu" and isinstance(array, cp.ndarray):
            array = da.from_array(cp.asnumpy(array), chunks=self.chunks)
        return array
    ...
    def run_benchmark(self, duration=2, verbose=True):
        ...
        for dtype in self.dtypes:
            ...
            for i, var_kwargs1 in enumerate(pdict):
                ...
                var_kwargs_dask_cpu = self._update_kwargs_arrays(var_kwargs1, "dask-cpu")
                ...
                kw_dask_cpu = {**self.fixed_kwargs_dask_cpu, **var_kwargs_dask_cpu}
                ...
                rep_kwargs_dask_cpu = self.get_reps(
                    self.func_dask_cpu, self.args_dask_cpu, kw_dask_cpu, duration, cpu=True
                )
                ...
                perf_dask_cpu = repeat_dask(self.func_dask_cpu, self.args_dask_cpu, kw_dask_cpu, **rep_kwargs_dask_cpu)
                ...
                df.at[index, "Dask accel"] = perf.cpu_times.mean() / perf_dask_cpu.gpu_times.mean()
                ...
                df.at[index, "Dask (CPU): host (mean)"] = perf_dask_cpu.cpu_times.mean()
                df.at[index, "Dask (CPU): host (std)"] = perf_dask_cpu.cpu_times.std()
        ...
        results["module_name_dask"] = self.module_name_dask
