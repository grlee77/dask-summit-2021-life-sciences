...
if n_gpus == 0:
    ...
    rects1 = ax.bar(x - width / 2, greg_cpu_means, width, label='without apply_parallel', color='#FFB043')
    rects2 = ax.bar(x + width / 2, greg_dask_cpu_means_main, width, label='with apply_parallel', color='#6A16F8')

else:
    raise NotImplementedError("no GPU models have results stored in this script")


# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Duration (s)', fontdict=dict(fontweight='bold', fontsize=font_size))
